...
# Modified by Katty Huang 
# - for use with onebasis only (no lag dimension)
#   -> therefore only dir="forw" is allowed
# - allows no cases input for type="af"

################################################################################
attrdl <- function(x,basis,cases=NULL,model=NULL,coef=NULL,vcov=NULL,type="af",
  dir="forw",tot=TRUE,cen,range=NULL,sim=FALSE,nsim=5000) {
################################################################################
#
...
#
  ...
  if((type=="an")|(!is.null(cases))) {
	  if(NROW(cases)!=NROW(x)) stop("'x' and 'cases' not consistent")
          if(NCOL(cases)>1L) {
